...
Testování materiálů 
metodou AE můžeme formulovat 
dvěma základními způsoby; při~\textbf{dopředné úloze} je 
stanovována odezva dle známých vstupů (sil nárazů). 
Nelze-li snadno získat hodnotu vstupu, 
je formulován problém zpětný – ze~změřené odezvy se 
dopočítávají hodnoty vstupů. 
Tento způsob řešení nazveme \textbf{inverzním algoritmem} \cite{Benes_podklad_advances}.
...
\section{Aplikace strojového učení při testování metodou 
AE v~komerční sféře}
\label{Sec:Kap1.5}
...
Někteří výrobci ultrazvukových snímačů a defektoskopů, 
jako například Olympus, 
se proto~úzce specializují i~na~diagnostiku metodou AE. 

Velkým výrobcem, 
který je kromě~diagnostiky materiálů dříve zmíněnými metodami 
(ultrazvukové
...
Konkuruje jim menší německá firma, která vyrábí rovněž kvalitní a~uznávané
  ...
  je Vallen Systeme. Tito dva nejvýznamnější výrobci kromě toho ladí 
  a~vyvíjejí software k automatické a efektivnější 
  klasifikaci dat pomocí strojového učení.
...
Program od firmy Vallen se jmenuje VisualClass. 
Jak je z názvu přímo patrné, jedná se primárně o nástroj
pro automatickou klasifikaci dat~–~každé vlně přidělí určité číslo třídy. 
V každé z těchto přidělených tříd klasifikuje podobnost se vzorovými 
daty. 
Funkcí, která představuje míru podobnosti mezi vlastní 
změřenou vlnou a vzorem, je \textbf{poměr vzdáleností}. 

Čím nižší je poměr vzdáleností, tím lépe odpovídá 
aktuální průběh přidělené třídě. 
Výsledky těchto klasifikátorů mohou být pro~každý dílčí 
detekovaný zásah v~programu VisualAE
(program od společnosti Vallen pro analýzu dat AE. 
Zpracovává jak dvojrozměrná, tak trojrozměrná data, 
počítá hodnoty nejistot měření, atd.) 
spojeny s odpovídajícími parametry. 
Klasifikované výsledky lze tak rovnou i~statisticky zpracovat.

Výše zmíněný klasifikátor 
(čili kritérium, kterým jsou vlnové průběhy přidělovány do tříd) 
lze chápat jako výsledek trénování modelu nebo také procesu učení modelu. 
V rámci procesu učení je brán určitý počet datových (učících) sad. 
Byla-li data vybrána uživatelem, hovoří se o \textbf{řízeném učení} 
(supervised learning). 
Pokud byly tyto sady vybrány 
automaticky nějakým výkonným algoritmem tzv. clusteringu (shromažďování dat,
"shlukování"), 
...
\begin{figure}[htbp]
    ...
    \caption{Zobrazení "příznaku proti příznaku" (feature-feature projekce) \cite{vallen_visual_class}}
...
schopnost zobrazení příznaku proti příznaku (feature-feature projekce). 
Jde o zobrazení 
závislosti jednoho příznaku na druhém příznaku – jedné extrahované
veličiny (příznaku) na druhé extrahované veličině. 
...
tak mezi několika třídami. Zobrazení feature-feature demonstruje 
...
rozpoznávání vzorů (pattern recognition) a práci s neuronovými sítěmi podobně,
jako VisualClass. Jedná se tak podobně o software sloužící 
k~automatické klasifikaci dat. Provádí korelaci jednotlivých příznaků - 
...
NOESIS umí základní signálové manipulace a výpočet jednoduchých operací, jako jsou FFT nebo~RMS,
...
Z kapitoly \ref{fig:noesis_extrahovany_priznak} plyne, že se v současné době algoritmy strojového 
učení komerčně využívají spíše pro~automatickou klasifikaci 
naměřených dat AE, než pro~přímé lokalizování zdrojů AE.
Techniky strojového učení ale přesto do budoucna 
reprezentují alternativní přístup k tradičním
metodám lokalizace zdrojů AE. Navzdory potřebě velkého množství dat
pro~trénink algoritmů mohou techniky strojového učení testování materiálů metodou AE
optimalizovat.
Vlastní výkon modelu založeného na strojovém učení závisí především
na~datovém setu, který byl použit pro učení \cite{Cia}. Jak již bylo dříve uvedeno, 
...
Velkou nevýhodou modelů strojového učení je také náchylnost k~poruše.
...
$u_i\dots$ hodnota napětí pro daný vzorek i (spočtená z periferní hodnoty dle vztahu -- dodat VZTAH) [V]
\begin{equation}
    \label{eq:ASL}
...
\end{equation}
Snahou bylo vybrat jeden nejlepší průběh z~každé sady, pro~který by 
